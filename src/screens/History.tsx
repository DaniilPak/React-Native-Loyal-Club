...
import { getBusinessClients, getBusinessInfoByBid, getBusinessReceipts } from '../utils/api';
...
function History({ route }: SettingsProps) {
  ...
  const initFunc = async () => {
    try {
      ...
      const businessReceipts = await getBusinessReceipts(businessId);
  ...
  useEffect(() => {
    ...
    navigation.setOptions({
      headerRight: () => <PressableIcon onPress={() => openFilterModal()} icon="filter" />,
    });
  }, []);
